...
class SemanticAlignmentAgent:
        ...
        if enable_llm:
            try:
                ...
                if self.llm_client.is_available():
                    ...
                    self.logger.info(
                        "LLM-enhanced semantic alignment agent initialized"
                    )
                else:
                    self.logger.warning(
                        "LLM client not available, using rule-based alignment only"
                    )
    ...
    def align_elements(
        ...
        for element in ifc_elements:
            if element.ifc_type in [IFCElementType.SLAB, IFCElementType.SPACE]:
                result = self._align_category_a_enhanced(
                    element, regulation_data, context
                )
        ...
        if self._needs_llm_enhancement(traditional_result):
            if self.enable_llm and self.llm_client and self.llm_client.is_available():
                try:
                    ...
                    self.logger.warning(
                        f"LLM enhancement failed: {e}, using traditional result"
                    )
    ...
    def _needs_llm_enhancement(self, result: AlignmentResult) -> bool:
        """判断是否需要LLM增强"""
        return (
            result.confidence < 0.7  # 置信度较低
            or len(result.alternatives) > 1  # 有多个备选方案
            or result.requires_review  # 需要人工审查
        )
    ...
    def _llm_enhanced_category_a_alignment(
        ...
        llm_context = {
            ...
            "geometric_features": {
                "area": (
                    element.geometric_features.area if element.geometric_features else 0
                ),
                "thickness": (
                    element.geometric_features.thickness
                    if element.geometric_features
                    else 0
                ),
                "position": (
                    str(element.geometric_features.position)
                    if element.geometric_features
                    else "Unknown"
                ),
            },
        ...
        try:
            ...
            if llm_confidence > traditional_result.confidence:
                ...
                return AlignmentResult(
                    ...
                    regulation_category=self._extract_regulation_category_from_llm(
                        llm_analysis
                    ),
                    function_classification=self._extract_function_classification_from_llm(
                        llm_analysis
                    ),
                    confidence=min(llm_confidence, 0.95),
                    confidence_level=self._get_confidence_level(llm_confidence),
                    reasoning_path=traditional_result.reasoning_path
                    + [f"LLM enhancement: {llm_analysis[:100]}..."],
                ...
                return AlignmentResult(
                    ...
                    confidence_level=self._get_confidence_level(
                        traditional_result.confidence + 0.1
                    ),
                    reasoning_path=traditional_result.reasoning_path
    ...
    def _extract_regulation_category_from_llm(
        self, analysis: str
    ) -> RegulationCategory:
        """从LLM分析中提取法规分类"""
        ...
        if any(
            ...
            for keyword in ["residential", "dwelling", "apartment"]
        ):
            return RegulationCategory.RESIDENTIAL
        elif any(
            keyword in analysis_lower
            for keyword in ["office", "commercial", "business"]
        ):
            return RegulationCategory.OFFICE
        elif any(
            keyword in analysis_lower
            for keyword in ["industrial", "factory", "manufacturing"]
        ):
    ...
    def _align_category_a(
        ...
        if element.ifc_type == IFCElementType.SLAB:
            ...
            return self._align_a2_space_classification(
                element, regulation_data, context
            )
        else:
            # 默认处理
            return self._create_default_result(
                element, AlignmentType.CATEGORY_A_FUNCTIONAL
            )
    ...
    def _align_a1_slab_classification(
        ...
        # 1. 评估功能证据
        thickness = (
            element.geometric_features.thickness if element.geometric_features else 0.0
        )
        ...
        if element.geometric_features:
            # 检查是否在屋顶
            if element.geometric_features.elevation > context.floor_height * (
                context.total_floors - 0.5
            ):
        ...
        if element.function_inference:
            if "equipment" in element.function_inference.primary_function.lower():
                ...
                reasoning_path.append(
                    f"功能推断: {element.function_inference.primary_function} → 设备支撑"
                )
                ...
                support_confidence = element.function_inference.confidence
                reasoning_path.append(
                    f"功能推断: {element.function_inference.primary_function} → 结构支撑"
                )
        ...
        if thickness_category == "decoration_platform":
            ...
            thickness_category == "equipment_slab"
            or location_category == "rooftop_equipment"
        ):
        ...
        if (
            thickness_category == "equipment_slab"
            and location_category == "rooftop_equipment"
        ):
        ...
        requires_review = (
            confidence_level == ConfidenceLevel.LOW or final_confidence < 0.7
        )
    ...
    def _align_a2_space_classification(
        ...
        if element.function_inference:
            ...
            reasoning_path.append(
                f"功能推断: {primary_function} (置信度: {function_confidence:.2f})"
            )
            evidence.extend(element.function_inference.evidence)
        ...
        if element.geometric_features:
            ...
            if element.geometric_features.height < 2.2:
                ...
                reasoning_path.append(
                    f"几何验证: 层高 {element.geometric_features.height:.2f}m < 2.2m → 辅助用房"
                )
                evidence.append(
                    f"space_height: {element.geometric_features.height:.2f}m"
                )

            # 记录面积信息（不进行面积计算）
            if element.geometric_features.area:
                reasoning_path.append(
                    f"几何信息: 面积 {element.geometric_features.area:.2f}m²"
                )
        ...
        function_classification = (
            primary_function if primary_function != "unknown" else "unclassified_space"
        )
        regulation_category = RegulationCategory.INCLUDE_FULL
        ...
        if any(keyword in primary_function for keyword in special_spaces):
            ...
            "mechanical" in primary_function.lower()
            or "equipment" in primary_function.lower()
        ):
            ...
            "office" in primary_function.lower()
            or "habitable" in primary_function.lower()
        ):
        ...
        confidence_level = self._get_confidence_level(final_confidence)
        requires_review = (
            confidence_level == ConfidenceLevel.LOW or final_confidence < 0.6
        )
    ...
    def _align_b1_vertical_openings(
        ...
        if cross_section_consistency > 0.7:
            reasoning_path.append(
                f"截面一致性: {cross_section_consistency:.2f} → 同一开口实例"
            )
            evidence.append(
                f"cross_section_consistency: {cross_section_consistency:.2f}"
            )
    ...
    def _align_b2_space_classification(
        ...
        if space.geometric_features:
            ...
            reasoning_path.append(f"几何信息: 面积 {area:.2f}m², 高度 {height:.2f}m")
            evidence.extend(
                [f"space_area: {area:.2f}m²", f"space_height: {height:.2f}m"]
            )
        ...
        requires_review = (
            ...
            or space_type == VerticalSpaceType.UNKNOWN
        )
    ...
    def _create_default_result(
        self, element: IFCElementInfo, alignment_type: AlignmentType
    ) -> AlignmentResult:
        """创建默认对齐结果"""
        return AlignmentResult(
            element_guid=element.guid,
            alignment_type=alignment_type,
            regulation_category=RegulationCategory.INCLUDE_FULL,
            function_classification="unknown",
            confidence=0.5,
    ...
    def generate_alignment_report(
        ...
        report = {
            "summary": {
                ...
                "category_a_count": len(
                    [
                        r
                        for r in results
                        if r.alignment_type == AlignmentType.CATEGORY_A_FUNCTIONAL
                    ]
                ),
                "category_b_count": len(
                    [
                        r
                        for r in results
                        if r.alignment_type == AlignmentType.CATEGORY_B_GEOMETRIC
                    ]
                ),
                "high_confidence_count": len(
                    [r for r in results if r.confidence_level == ConfidenceLevel.HIGH]
                ),
            ...
            "confidence_distribution": {
                ...
                "medium": len(
                    [r for r in results if r.confidence_level == ConfidenceLevel.MEDIUM]
                ),
                "low": len(
                    [r for r in results if r.confidence_level == ConfidenceLevel.LOW]
                ),
            },
            "regulation_categories": {
                "include_full": len(
                    [
                        r
                        for r in results
                        if r.regulation_category == RegulationCategory.INCLUDE_FULL
                    ]
                ),
                "include_partial": len(
                    [
                        r
                        for r in results
                        if r.regulation_category == RegulationCategory.INCLUDE_PARTIAL
                    ]
                ),
                "exclude": len(
                    [
                        r
                        for r in results
                        if r.regulation_category == RegulationCategory.EXCLUDE
                    ]
                ),
                "deduct_per_floor": len(
                    [
                        r
                        for r in results
                        if r.regulation_category == RegulationCategory.DEDUCT_PER_FLOOR
                    ]
                ),
            },
            "detailed_results": [
