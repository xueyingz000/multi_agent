...
from ..utils import (
    log, GeometricFeatures, IfcElementInfo, FunctionInference, 
...
class FunctionInferenceEngine:
        ...
        if enable_llm:
            try:
                ...
                if self.llm_client.is_available():
                    ...
                    log.warning("LLM client not available, using rule-based inference only")
    ...
    def _initialize_rules(self) -> List[InferenceRule]:
        ...
        return [
            ...
            InferenceRule(
                name="equipment_platform_medium",
                conditions={
                    'ifc_type': 'IfcSlab',
                    'thickness_min': 0.1,
                    'thickness_max': 0.15,
                    'location_indicator': ['rooftop_equipment', 'intermediate_equipment']
                },
                function_type=FunctionType.EQUIPMENT_PLATFORM,
                confidence_base=0.75,
                evidence_keywords=['equipment', 'platform', 'mechanical', 'rooftop'],
                priority=2
            ),
            InferenceRule(
            ...
            # IfcSpace 推断规则
            InferenceRule(
                name="habitable_office_space",
                conditions={
                    'ifc_type': 'IfcSpace',
                    'height_min': 2.2,
                    'location_indicator': ['habitable_space']
                },
                function_type=FunctionType.HABITABLE_OFFICE,
                confidence_base=0.8,
                evidence_keywords=['office', 'room', 'habitable', 'workspace'],
                priority=1
            ),
            InferenceRule(
                name="mechanical_equipment_room",
                conditions={
                    'ifc_type': 'IfcSpace',
                    'equipment_proximity': True
                },
                function_type=FunctionType.MECHANICAL_ROOM,
                confidence_base=0.75,
                evidence_keywords=['mechanical', 'equipment', 'plant', 'utility'],
                priority=2
            ),
            InferenceRule(
                name="auxiliary_low_space",
                conditions={
                    'ifc_type': 'IfcSpace',
                    'height_max': 2.2,
                    'location_indicator': ['low_auxiliary_space']
                },
                function_type=FunctionType.AUXILIARY_ROOM,
                confidence_base=0.7,
                evidence_keywords=['auxiliary', 'storage', 'utility', 'service'],
                priority=3
            ),
            InferenceRule(
                ...
                conditions={
                    ...
                    'location_indicator': ['outdoor_construction', 'auxiliary_construction']
                },
                function_type=FunctionType.AUXILIARY_CONSTRUCTION,
                confidence_base=0.8,
                evidence_keywords=['outdoor', '消防', '水池', '停车', '货棚', '垃圾', '加油', '收费'],
                priority=1
            ),
    ...
    def infer_function(self, element_info: IfcElementInfo, 
                      geometric_features: GeometricFeatures,
                      spatial_context: Optional[SpatialContext] = None) -> FunctionInference:
        """推断元素功能"""
        try:
            # 收集所有匹配的规则
            matching_rules = self._find_matching_rules(element_info, geometric_features, spatial_context)
            
            if not matching_rules:
                return self._create_default_inference(element_info)
            
            # 按优先级排序
            matching_rules.sort(key=lambda x: (x[0].priority, -x[1]))  # 优先级升序，置信度降序
            ...
            adjusted_confidence = self._adjust_confidence(
                base_confidence, element_info, geometric_features, spatial_context, best_rule
            )
            ...
            # 生成备选方案
            alternatives = self._generate_alternatives(matching_rules[1:5])  # 最多5个备选
            
            inference = FunctionInference(
                primary_function=best_rule.function_type,
                confidence=min(0.95, max(0.1, adjusted_confidence)),  # 限制在0.1-0.95之间
                evidence=evidence,
                alternatives=alternatives,
                reasoning_path=self._generate_reasoning_path(best_rule, element_info, geometric_features)
            )
            
            log.debug(f"Inferred function for {element_info.guid}: {best_rule.function_type.value} (confidence: {inference.confidence:.3f})")
            return inference
    ...
    def infer_function_enhanced(
        ...
        """
        # 首先进行传统规则推断
        rule_based_result = self.infer_function(element_info, geometric_features, spatial_context)
            ...
            if self.enable_llm and self.llm_client and self.llm_client.is_available():
                try:
                    return self._llm_enhanced_inference(
                        ...
                        building_context, rule_based_result
                    )
                except Exception as e:
    ...
    def _needs_llm_analysis(self, rule_based_result: FunctionInference) -> bool:
        ...
        """
        return (
            rule_based_result.confidence < 0.6 or  # 置信度阈值
            len(rule_based_result.alternatives) > 1 or
            len(rule_based_result.evidence) < 2
        )
    ...
    def _llm_enhanced_inference(
        ...
        context = {
            ...
            'spatial_context': {
                'adjacent_spaces': str(spatial_context.adjacent_spaces if spatial_context else []),
                'floor_level': spatial_context.floor_level if spatial_context else 0,
                'support_relationships': str(spatial_context.support_relationships if spatial_context else [])
            },
            ...
            'rule_based_analysis': {
                ...
                'alternatives': [(alt[0].value, alt[1]) for alt in rule_based_result.alternatives],
        ...
        # 解析LLM结果并与规则推断结合
        return self._combine_llm_and_rule_results(llm_result, rule_based_result, context)
    ...
    def _combine_llm_and_rule_results(
        ...
        try:
            ...
            if llm_confidence > rule_based_result.confidence:
                # 尝试从LLM分析中提取功能类型
                predicted_function = self._extract_function_from_llm_analysis(llm_analysis)
                ...
                return FunctionInference(
                    ...
                    alternatives=rule_based_result.alternatives + [(rule_based_result.primary_function, rule_based_result.confidence)],
                ...
                return FunctionInference(
                    ...
                    confidence=min(rule_based_result.confidence + 0.1, 0.95),  # 轻微提升置信度
                    evidence=enhanced_evidence,
    ...
    def _extract_function_from_llm_analysis(self, llm_analysis: str) -> FunctionType:
        ...
        function_keywords = {
            FunctionType.STRUCTURAL_FLOOR: ['structural', 'load bearing', 'support', 'beam', 'foundation'],
            FunctionType.EQUIPMENT_PLATFORM: ['equipment', 'platform', 'mechanical', 'rooftop', 'hvac'],
            FunctionType.DECORATIVE_PLATFORM: ['decorative', 'decoration', 'thin', 'canopy', 'aesthetic'],
            FunctionType.HABITABLE_OFFICE: ['office', 'habitable', 'workspace', 'room', 'occupancy'],
            FunctionType.MECHANICAL_ROOM: ['mechanical', 'equipment room', 'plant', 'utility', 'service'],
            FunctionType.AUXILIARY_ROOM: ['auxiliary', 'storage', 'utility', 'service', 'support'],
            FunctionType.AUXILIARY_CONSTRUCTION: ['outdoor', 'construction', 'parking', 'garage'],
            FunctionType.ATRIUM: ['atrium', 'lobby', 'hall', 'void', 'multi-story'],
            FunctionType.SHAFT: ['shaft', 'duct', 'pipe', 'elevator', 'stair', 'vertical']
        }
    ...
    def _find_matching_rules(self, element_info: IfcElementInfo, 
                           geometric_features: GeometricFeatures,
                           spatial_context: Optional[SpatialContext] = None) -> List[Tuple[InferenceRule, float]]:
        """查找匹配的规则"""
        matching_rules = []
        
        for rule in self.inference_rules:
            match_score = self._evaluate_rule_match(rule, element_info, geometric_features, spatial_context)
            if match_score > 0:
                matching_rules.append((rule, rule.confidence_base * match_score))
        
        return matching_rules
    
    def _evaluate_rule_match(self, rule: InferenceRule, 
                           element_info: IfcElementInfo,
                           geometric_features: GeometricFeatures,
                           spatial_context: Optional[SpatialContext] = None) -> float:
        """评估规则匹配度"""
        ...
        if 'height_min' in conditions:
            if geometric_features.thickness < conditions['height_min']:  # 对空间，thickness表示高度
                match_score *= 0.5
        ...
        if spatial_context:
            if 'equipment_proximity' in conditions:
                if spatial_context.equipment_nearby != conditions['equipment_proximity']:
                    match_score *= 0.8
        ...
        if 'floors_spanned_min' in conditions:
            floors_spanned = getattr(geometric_features, 'vertical_span', {}).get('floors_spanned', 1)
    ...
    def _adjust_confidence(self, base_confidence: float, 
                          ...
                          geometric_features: GeometricFeatures,
                          spatial_context: Optional[SpatialContext],
                          rule: InferenceRule) -> float:
        """调整置信度"""
        adjusted = base_confidence
        
        # 几何一致性调整
        geometric_consistency = self._evaluate_geometric_consistency(geometric_features, rule)
        adjusted += geometric_consistency * self.confidence_adjustments['geometric_consistency']
        ...
        # 空间上下文调整
        if spatial_context:
            context_match = self._evaluate_spatial_context_match(spatial_context, rule)
            adjusted += context_match * self.confidence_adjustments['spatial_context']
        
        # 属性证据调整
        property_evidence = self._evaluate_property_evidence(element_info, rule)
        adjusted += property_evidence * self.confidence_adjustments['property_evidence']
        
        return adjusted
    
    def _evaluate_geometric_consistency(self, geometric_features: GeometricFeatures, 
                                      rule: InferenceRule) -> float:
        """评估几何一致性"""
        ...
        # 支撑功能一致性
        if rule.function_type in [FunctionType.STRUCTURAL_FLOOR, FunctionType.HABITABLE_OFFICE]:
            if geometric_features.support_function == 'supports_occupancy':
                consistency_score += 0.3
        elif rule.function_type in [FunctionType.EQUIPMENT_PLATFORM, FunctionType.MECHANICAL_ROOM]:
            if geometric_features.support_function == 'supports_equipment':
                consistency_score += 0.3
        
        return min(1.0, consistency_score)
    
    def _evaluate_keyword_match(self, element_info: IfcElementInfo, rule: InferenceRule) -> float:
        """评估关键词匹配"""
        text_sources = [
    ...
    def _evaluate_spatial_context_match(self, spatial_context: SpatialContext, 
                                      rule: InferenceRule) -> float:
        """评估空间上下文匹配"""
        match_score = 0.0
        
        # 设备邻近性
        if rule.function_type in [FunctionType.EQUIPMENT_PLATFORM, FunctionType.MECHANICAL_ROOM]:
    ...
    def _evaluate_property_evidence(self, element_info: IfcElementInfo, rule: InferenceRule) -> float:
        """评估属性证据"""
        evidence_score = 0.0
    ...
    def _collect_evidence(self, element_info: IfcElementInfo,
                         geometric_features: GeometricFeatures,
                         spatial_context: Optional[SpatialContext],
                         rule: InferenceRule) -> List[Evidence]:
        """收集证据"""
        evidence = []
        ...
        for prop_name, prop_value in element_info.properties.items():
            if prop_name in ['Name', 'Description', 'LongName', 'ObjectType', 'PredefinedType']:
                if prop_value and any(keyword.lower() in str(prop_value).lower() 
                                    for keyword in rule.evidence_keywords):
                    evidence.append(Evidence(
    ...
    def _generate_alternatives(self, other_rules: List[Tuple[InferenceRule, float]]) -> List[Tuple[FunctionType, float]]:
        """生成备选方案"""
        alternatives = []
    ...
    def _generate_reasoning_path(self, rule: InferenceRule, 
                               element_info: IfcElementInfo,
                               geometric_features: GeometricFeatures) -> str:
        """生成推理路径"""
        path_parts = [
            f"应用规则: {rule.name}",
    ...
    def _create_default_inference(self, element_info: IfcElementInfo) -> FunctionInference:
        """创建默认推断结果"""
        # 基于IFC类型的默认推断
        if element_info.ifc_type == 'IfcSlab':
            default_function = FunctionType.STRUCTURAL_FLOOR
        elif element_info.ifc_type == 'IfcSpace':
            default_function = FunctionType.HABITABLE_OFFICE
        else:
            default_function = FunctionType.STRUCTURAL_FLOOR
    ...
    def batch_infer_functions(self, elements_data: Dict[str, Tuple[IfcElementInfo, GeometricFeatures]],
                            spatial_context: Optional[SpatialContext] = None) -> Dict[str, FunctionInference]:
        """批量推断功能"""
        results = {}
        
        for guid, (element_info, geometric_features) in elements_data.items():
            try:
                inference = self.infer_function(element_info, geometric_features, spatial_context)
